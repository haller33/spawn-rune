...
int creaddir_files_linear(char *name_dir, char **files_arr_ret,
  ...
  // not include . .. folders on list, that a ready have count on
  // count_files_dir()
  files_arr = (char *)calloc(sizeof(char *), files_count * MAX_CARACTER_OF_FILE);
  ...
  while ((entry = readdir(folder))) {
    ...
    if (!(entry->d_type == DT_DIR) &&
        (entry->d_off > 2)) { // ignore default linux directory [. ..]
        strcpy(&files_arr[i * MAX_CARACTER_OF_FILE], entry->d_name);
      i = i + 1;
    }
  }
...
int main() {
  ...
  creaddir_files_linear(directory, &files_arr, &size_files);
  ...
  for (i = 0; i < size_files; i++) {
    ...
    // printf("file(%d) : %s\n", i, files_arr[i]);
    // if(files_arr[i] == NULL) continue;
    printf("%s\n", &files_arr[i*MAX_CARACTER_OF_FILE]);
  }
#endif
  ...
  #if true
  free_read_dir_linear(files_arr, size_files);
  #endif

  return 0;
}
#endif // HAVE_MAIN_LINEAR
